...
\caption{The simulated solution when $\mu=1$. Stable solution at approximately $2000000$ time steps.}
\end{subfigure}
~
\begin{subfigure}[b]{0.48\textwidth}
...
\end{subfigure}
\caption{Figure shows the stable solutions found by the simulation on a homogeneous grid containing $1024$ nodes where each node has been perturbed a small amount. The pink lines show the conductivity along each edge, with the thickness of the line is proportional to the amount of conductivity. The shortest path found with Dijkstra's algorithm from each source to each sinks has been marked with a thin white line \cite{Dijkstra}. The parameters used are $q = 10^{-4},\lambda = 10^{-3},$ and $D_{min}=5 \cdot 10^{-2}$ with a time step of one. In this graph there are in total one sink and $20$ sources. Each source (red) produces 1000 particles each time unit and all sinks (blue) can together remove as many particles as are produced each time unit (evenly divided between the sinks).}
\label{fig:sources}
\end{figure}
...
\caption{The simulated solution when $\mu=1.4$. Stable solution at approximately $xxxx$ time steps.}
\end{subfigure}
~
\begin{subfigure}[b]{0.48\textwidth}
\includegraphics[width=\textwidth]{{img/nodes_sinks_M10_mu1.5}.png}
\caption{The simulated solution when $\mu=1.5$. Stable solution at approximately $xxxx$ time steps.}
\end{subfigure}
\caption{Figure shows the stable solutions found by the simulation on a homogeneous grid containing $1024$ nodes where each node has been perturbed a small amount. The pink lines show the conductivity along each edge, with the thickness of the line is proportional to the amount of conductivity. The shortest path found with Dijkstra's algorithm from each source to each sinks has been marked with a thin white line \cite{Dijkstra}. The parameters used are $q = 10^{-4},\lambda = 10^{-3},$ and $D_{min}=5 \cdot 10^{-2}$ with a time step of one. In this graph there are in total one source and $20$ sinks. Each source (red) produces 1000 particles each time unit and all sinks (blue) can together remove as many particles as are produced each time unit (evenly divided between the sinks).}
\label{fig:sinks}
\end{figure}
